...
class InstanceInfoProcessor:
    ...
    def _process_profit_loss_bounds(self):
        ...
        if self._db_trade_performance.profit > self._instance_info[InstanceInfoProcessor.MAX_PROFIT]:
            self._instance_info[InstanceInfoProcessor.MAX_PROFIT] = self._db_trade_performance.profit

        if self._db_trade_performance.profit < self._instance_info[InstanceInfoProcessor.MAX_LOSS]:
            self._instance_info[InstanceInfoProcessor.MAX_LOSS] = self._db_trade_performance.profit

        pnl_ratio = self._db_trade_performance.profit / self._db_trade_performance.available_capital
        if pnl_ratio > self._instance_info[InstanceInfoProcessor.MAX_PROFIT_PERCENT]:
            self._instance_info[InstanceInfoProcessor.MAX_PROFIT_PERCENT] = pnl_ratio

        if pnl_ratio < self._instance_info[InstanceInfoProcessor.MAX_LOSS_PERCENT]:
            self._instance_info[InstanceInfoProcessor.MAX_LOSS_PERCENT] = pnl_ratio

    def _process_roi_percent(self):
        self._maybe_populate(InstanceInfoProcessor.ROI_PERCENT, 0)
        self._instance_info[InstanceInfoProcessor.ROI_PERCENT] += self._db_trade_performance.profit / self._db_trade_performance.available_capital
